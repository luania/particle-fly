    ...
    try {
        ...
        editor_1.getBuffer().onDidChangeText(function (ev) {
            for (var _i = 0, _a = ev.changes; _i < _a.length; _i++) {
                var e = _a[_i];
                var position = e.newRange[e.newText ? "end" : "start"];
                var aPos = editorElement_1.pixelPositionForScreenPosition(editor_1.screenPositionForBufferPosition(position));
                var rect = scrollView_1.getBoundingClientRect();
                var left = aPos.left
                    + rect.left
                    - editorElement_1.getScrollLeft();
                var top_1 = aPos.top
                    + rect.top
                    - editorElement_1.getScrollTop()
                    + editor_1.getLineHeightInPixels();
                particleSystem.originPosition.x = left;
                particleSystem.originPosition.y = top_1;
                if (conf.alwaysEmit) {
                    return;
                }
                if (!loadding) {
